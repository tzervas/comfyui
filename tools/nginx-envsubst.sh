...
: "${COMFYUI_UPSTREAM:=comfyui:8188}"
: "${OPENWEBUI_UPSTREAM:=openwebui:8080}"
: "${PIPELINES_UPSTREAM:=openwebui-pipelines:9099}"

export STACK_FQDN MTLS_VERIFY OLLAMA_UPSTREAM COMFYUI_UPSTREAM OPENWEBUI_UPSTREAM PIPELINES_UPSTREAM SSO_ENABLED OAUTH2_PROXY_UPSTREAM
...
envsubst '${STACK_FQDN} ${MTLS_VERIFY} ${OLLAMA_UPSTREAM} ${COMFYUI_UPSTREAM} ${OPENWEBUI_UPSTREAM} ${PIPELINES_UPSTREAM} ${OAUTH2_PROXY_UPSTREAM} ${SSO_LOCATIONS} ${AUTH_GATE} ${MTLS_CA_DIRECTIVE} ${REGISTRY_MTLS_ENFORCE} ${REGISTRY_SECRET} ${MODEL_VAULT_API_PROXY_PASS} ${MODEL_VAULT_REGISTER_PROXY_PASS} ${MODEL_VAULT_DISCOVER_PROXY_PASS} ${MODEL_VAULT_PROXY_SSL}' \
